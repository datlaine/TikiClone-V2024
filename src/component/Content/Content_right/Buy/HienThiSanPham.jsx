...
export default function HienThiSanPham({sanPham}) {
  ...
  useEffect(() => {
      ...
      setProduct((prev) => ({
        ...
        isPricePromote:
          Number(
              sanPham.current.isPrice - (sanPham.current.isPrice / 100) * Number(sanPham.current?.isPromote[1]?.promote) * -1,
            )
          || 0,
          }))
                ...
                <div className={style.wrapperPriceAstra}>
                  ...
                  >
                    ...
                    <span className={style.priceReal}>
                      {sanPham.current?.isPromote[0]?.checkPromote ? sanPham.current?.isPromote[1]?.promote + '%' : ''}
                    </span>
                    <span>
