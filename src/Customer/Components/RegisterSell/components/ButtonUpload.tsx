...
import { EyeIcon, MoreHorizontal, View, X } from 'lucide-react'
...
interface IProps {
    ...
    setUrlProductThumb: React.Dispatch<
        SetStateAction<{ product_id: string; product_thumb_image: { secure_url: string; public_id: string } }>
    >
}
...
const ButtonUpload = (props: IProps) => {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (filePreview || fileProduct) {
            ...
            setUrlProductThumb({ product_id: '', product_thumb_image: { secure_url: '', public_id: '' } })
        }
        if (e.target.files) {
