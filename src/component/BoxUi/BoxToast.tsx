    ...
    return (
        <>
            {show && (
                <p
                    className={`${
                        props.type === 'SUCCESS' ? ' text-green-900 ' : props.type === 'ERROR' ? ' text-red-900 ' : '  text-orange-900'
                    } 
bg-[#ffffff]  py-[24px] border-y-0 border-r-0 border-l-[4px] shadow-2xl relative w-[450px] min-h-[120px] h-auto rounded-lg transition-all duration-500  border-[2px] flex items-center justify-center`}
                >
                    ...
                    <span
                        className={`${
                            props.type === 'SUCCESS' ? 'border-green-500' : props.type === 'ERROR' ? 'border-red-500' : 'border-orange-500'
                    ...
                    >
                        <X
                            color={`${
                                props.type === 'SUCCESS' ? 'rgb(34 197 94)' : props.type === 'ERROR' ? ' rgb(239 68 68)' : 'rgb(249 115 22)'
                            }`}
                        />
                    </span>

                    <span className={`absolute flex justify-center  bottom-[5px] animate-pulse left-[50px] p-[4px] text-[12px]  `}>
                        @datlai304
                    </span>
                </p>
            )}
